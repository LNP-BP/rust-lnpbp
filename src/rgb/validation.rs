...
pub struct HistoryGraph {
    ...
    open: Vec<NodeIndex<DefaultIx>>,
}

impl HistoryGraph {
    fn add_open_seals(&mut self, from_transition: &Transition, from_node: NodeIndex<DefaultIx>) {
    ...
    pub fn new(genesis: Transition) -> Self {
        ...
        graph.add_open_seals(&genesis, genesis_node);
        ...
        let closing_indexes = self
            ...
            .try_fold(HashSet::new(), |mut to_close, node_index| {
                if let Some(HistoryGraphNode::Open(node_seal)) = self.graph.node_weight(*node_index) {
                    if closes.iter().any(|seal| seal.compare(node_seal)) {
